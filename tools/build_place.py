...
p=argparse.ArgumentParser();p.add_argument('--rojo',default=shutil.which('rojo'));args=p.parse_args()
shared={'$className':'Folder'}
for name in ['Config','Rules','Core','Art','ArtData','Tech','Catalog','Motion','Panels','UI','Guide','Audio']: shared[name]={'$path':'src/'+name+'.lua'}
...
project={'name':'Dustbound-0.5.0','tree':{
 '$className':'DataModel',
...
if not args.rojo: raise SystemExit('Project mapping written. Install Rojo and run: rojo build -o Dustbound-0.5.0.rbxlx')
subprocess.run([args.rojo,'build','default.project.json','-o','Dustbound-0.5.0.rbxlx'],cwd=ROOT,check=True)
