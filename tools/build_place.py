...
p=argparse.ArgumentParser();p.add_argument('--rojo',default=shutil.which('rojo'));p.add_argument('--output',default='Dustbound-0.9.0-FIRST-CONTACT.rbxlx');args=p.parse_args()
shared={'$className':'Folder'}
for name in ['Runtime','Config','RunSystems','ResearchWeb','Economy','Encounters','BattleView','Frontier','Rules','Core','Art','ArtData','Tech','Catalog','Animation','WeaponRig','UI','Guide','Audio']: shared[name]={'$path':'src/'+name+'.lua'}
...
project={'name':'Dustbound-0.9.0','tree':{
 '$className':'DataModel',
...
if not args.rojo: raise SystemExit('Project mapping written. Install Rojo and run: rojo build -o Dustbound-0.9.0-FIRST-CONTACT.rbxlx')
