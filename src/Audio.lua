...
local A={}
local map={shot="cannon",impact="impact",basehit="warning",upgrade="upgrade",deposit="deposit",accepted="supply",supply="supply",["end"]="victory"}
...
function A.new(C)
 local self={pool={},at={},clock=0,volume=.65,musicVolume=.25,mode="内置兼容提示音",music=nil}
 ...
 function self.settings(settings)
  self.volume=settings.sfx;self.musicVolume=settings.music
  if self.music then self.music.Volume=self.musicVolume*.35 end
 end
 function self.play(key)
  if self.volume<=0 then return end
  local delay=key=="drill" and .62 or key=="machine" and .16 or key=="cannon" and .3 or .12
 ...
 function self.update(dt,paused,s)
  ...
  if paused then for _,snd in pairs(self.pool) do pcall(function() snd:Stop() end) end end
  ...
  if type(id)=="number" and id>0 and not self.music then
   self.music=Instance.new("Sound");self.music.Name="Music";self.music.SoundId="rbxassetid://"..id;self.music.Looped=true;self.music.Volume=self.musicVolume*.35;self.music.Parent=root;pcall(function() self.music:Play() end)
  end
  if self.music then self.music.Volume=paused and 0 or self.musicVolume*.35 end
 end
 return self
end
return A
