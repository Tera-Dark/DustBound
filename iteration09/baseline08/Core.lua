...
local function spawn(s,C)
 ...
 -- Ground units only enter from the two ground edges. Flyers own the sky lane.
 local x,y;local side=n%2==0 and -1 or 1
 if kind=="flyer" then x=360+(n*137)%720;y=-24
 else x=side<0 and -30 or 1470;y=366+(n%3)*18 end
...
function G.step(s,C,R,dt)
 ...
 for _,e in ipairs(s.enemies) do if not e.dead then
  ...
  if not e.dead and dist(e,goal)<=range+.1 and e.attack<=0 then e.attack=d.interval;e.attackAt=s.elapsed;emit(s,"enemyattack",{id=e.id,enemyKind=e.kind,x=e.x,y=e.y,tx=720,ty=340});hurt(s,(d.attack+s.wave*d.attackWave)*s.mission.attack) end
 end end
