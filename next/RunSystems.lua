-- 0.7 development core; engine-independent. NOT yet wired into the 0.6.1 game.
...
local function newRobot(id)
 return {id=id,x=385,mineX=755+(id-1)*84,state="outbound",progress=0,cargo=0,trips=0}
end
function M.buy(s,id)
 if s.paused or (s.phase~="combat" and s.phase~="mining") then return false,"当前不可投资" end
 ...
 if id=="robots" and s.phase=="mining" then
  for i=#s.robots+1,M.stats(s).robots do s.robots[i]=newRobot(i) end
 end
 return true
end
...
function M.waveCleared(s,wave,lastWave)
 ...
 for i=1,M.stats(s).robots do s.robots[i]=newRobot(i) end
...
function M.rerollPrice(s) return s.rerolls==0 and 0 or 80+40*(s.rerolls-1) end
function M.reroll(s,token)
 if s.paused or not s.offer or s.offer.token~=token or s.rerolls>=3 then return false end
 ...
 for _,r in ipairs(s.robots) do
  ...
  for _=1,4 do
   ...
   if r.state=="drilling" then
    ...
    local target=r.state=="outbound" and r.mineX or 385;local distance=math.abs(target-r.x)
    ...
    r.progress=r.state=="outbound" and (r.x-385)/(r.mineX-385) or (r.mineX-r.x)/(r.mineX-385)
